...
void EEPROMClass::begin(size_t size, uint8_t sector_offset) {
    if ((size <= 0) || (size > 4096)) {
        size = 4096;
    }
	
	real_sector = (intptr_t)_sector - 4096*sector_offset;
    ...
    memcpy(_data, (void *)real_sector, _size);
...
/*
uint8_t EEPROMClass::largeRead(int const startOffset, int const size) {
    if(!_data){
        return 0;
    }
    uint8_t readBuf[size];
    for(int i = 0; i < size; i++){
        readBuf[i] = _data[startOffset + i];
    }
    return readBuf;
}
*/
...
bool EEPROMClass::commit() {
    ...
    flash_range_erase(real_sector - (intptr_t)XIP_BASE, 4096);
    flash_range_program(real_sector - (intptr_t)XIP_BASE, _data, _size);
