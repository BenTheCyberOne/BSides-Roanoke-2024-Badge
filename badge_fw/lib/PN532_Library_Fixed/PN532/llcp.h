...
public:
    ...
    *           < 0     failed
    */
	int8_t activate(uint8_t *uid, uint16_t timeout = 10);

    int16_t waitForConnection(uint8_t *buf, uint8_t bufLength, uint16_t timeout = LLCP_DEFAULT_TIMEOUT);
    ...
    */
    ...
    bool initPassive(uint8_t cardbaudrate, uint8_t *uid, uint8_t *uidLength, uint16_t timeout, bool inlist = true);

    bool finish(const uint8_t *rbuf, uint8_t hlen, const uint8_t *body, uint8_t blen);

    bool activateTarget(uint16_t timeout);
