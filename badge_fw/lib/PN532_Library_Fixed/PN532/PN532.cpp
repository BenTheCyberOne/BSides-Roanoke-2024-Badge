...
uint32_t PN532::readRegister(uint16_t reg)
{
    ...
    if (HAL(writeCommand)(pn532_packetbuffer, 3)) {
        Serial.println("readRegister write return 0");
        return 0;
    }
    // read data packet
    int16_t status = HAL(readResponse)(pn532_packetbuffer, sizeof(pn532_packetbuffer));
    if (0 > status) {
        Serial.println("readRegister readResponse return 0");
        return 0;
    }
    response = pn532_packetbuffer[0];
    return response;
...
uint32_t PN532::writeRegister(uint16_t reg, uint8_t val)
{
    ...
    if (HAL(writeCommand)(pn532_packetbuffer, 4)) {
        Serial.println("writeRegister return 0");
        return 0;
    }
...
bool PN532::setRFField(uint8_t autoRFCA, uint8_t rFOnOff)
{
    ...
    if (HAL(writeCommand)(pn532_packetbuffer, 3)) {
        Serial.println("setRFField returned false");
        return false;
    }
...
/*
********* BAUD RATES FOR PN532 *********
BR is the Baud Rate to be used during the activation
− 0x00 : 106 kbps,
− 0x01 : 212 kbps,
− 0x02 : 424 kbps.

FOR inListPassiveTarget ONLY:                        https://www.nxp.com/docs/en/user-guide/141520.pdf Page 115
...
bool PN532::readPassiveTargetID(uint8_t cardbaudrate, uint8_t *uid, uint8_t *uidLength, uint16_t timeout, bool inlist)
{
    ...
    if (HAL(writeCommand)(pn532_packetbuffer, 3)) {
        DMSG_STR("Fail to write in readPassiveTargetID");
        return 0x0;  // command failed
    }
    DMSG_STR("Wrote command for reading passive target!");
    // read data packet
    if (HAL(readResponse)(pn532_packetbuffer, sizeof(pn532_packetbuffer), timeout) < 0) {
        DMSG_STR("Fail to read in readPassiveTargetID");
        return 0x0;
    }
    DMSG_STR("Read response from passive target!");
    status = true;
    ...
    */
    ...
    if (pn532_packetbuffer[0] != 1) {
        DMSG("PN532 did not find a tag in readPassiveTargetID");
        return 0;
    }
    ...
    *uidLength = pn532_packetbuffer[5];
    ...
    DMSG("STATUS IN PN532:");
    DMSG_STR(status);
    return 1;
}


bool PN532::inATR(uint16_t timeout)
{
    pn532_packetbuffer[0] = PN532_COMMAND_INATR;
    pn532_packetbuffer[1] = inListedTag;  //intended target (should really always be a value of 1)
    ...
    DMSG("Target has been activated!");
    return true;

}

bool PN532::inPSL(uint8_t targetTag, uint8_t newbaudrate, uint16_t timeout)
{
    pn532_packetbuffer[0] = PN532_COMMAND_INPSL;
    pn532_packetbuffer[1] = targetTag;  //intended target (should really always be 1)
    pn532_packetbuffer[2] = newbaudrate; //
    ...
    DMSG("Baudrate changed successfully");
    return true;
}
...
bool PN532::inDataExchange(uint8_t *send, uint8_t sendLength, uint8_t *response, uint8_t *responseLength)
{
  ...
  //writeCommand(header_buf, header_buf_len, body_buf, body_buf_len)
  if (HAL(writeCommand)(pn532_packetbuffer, 2, send, sendLength)) {
    return false;
  }
  //readResponse(res_buf, res_length, timeout)
  int16_t status = HAL(readResponse)(response, 5, 1100);
  if (status < 0) {
    DMSG_STR("Status indicated an error in inDataExchange");
    return false;
  }
...
int8_t PN532::tgInitAsTarget(const uint8_t* command, const uint8_t len, const uint16_t timeout) {
  ...
  if (status > 0) {
    ...
    //Serial.println("Read Failed! (else)");
    DMSG("Read failed\r\n");
    return -2;
  }
}
 ...
 * Peer to Peer
 */
int8_t PN532::tgInitAsTarget(uint8_t *uid, uint16_t timeout)
{
  const uint8_t command[] = {
      PN532_COMMAND_TGINITASTARGET,
      0,
      0x04, 0x00,         //SENS_RES (0x04 for 4 byte UID, 0x08 for 8)
      uid[0],uid[1],uid[2],   //NFCID1
      0x20,               //SEL_RES - typically 0x40 per https://www.nxp.com/docs/en/nxp/application-notes/AN133910.pdf - not sure this is true??

      0x01, 0xFE, 0x0F, 0xBB, 0xBA, 0xA6, 0xC9, 0x89, // FeliCaParams
...
int16_t PN532::tgGetData(uint8_t *buf, uint8_t len)
{
  ...
  int16_t status = HAL(readResponse)(buf, len);
  if (0 >= status) {
...
bool PN532::tgSetData(const uint8_t *header, uint8_t hlen, const uint8_t *body, uint8_t blen)
{
  if (hlen > (sizeof(pn532_packetbuffer) - 1)) {
    ...
    if(HAL(writeCommand)(pn532_packetbuffer,1)){
        DMSG_STR("tgSetData writeCommand failure");
      return false;
    }
  }
  else {
  ...
  if (0 > HAL(readResponse)(pn532_packetbuffer, sizeof(pn532_packetbuffer))) {
    return false;
    DMSG_STR("tgSetData readResponse failure");
  }

  if (0 != pn532_packetbuffer[0]) {
    return false;
  }
  DMSG_STR("tgSetData success!");
  return true;
}
