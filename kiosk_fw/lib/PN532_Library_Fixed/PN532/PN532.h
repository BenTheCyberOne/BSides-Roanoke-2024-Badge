...
public:
  ...
  *           < 0     failed
  */
  int8_t tgInitAsTarget( uint8_t *uid, uint16_t timeout = 100);
  int8_t tgInitAsTarget(const uint8_t* command, const uint8_t len, const uint16_t timeout = 100);
  ...
  bool inATR(uint16_t timeout);
  bool inPSL(uint8_t targetTag, uint8_t newbaudrate, uint16_t timeout);
