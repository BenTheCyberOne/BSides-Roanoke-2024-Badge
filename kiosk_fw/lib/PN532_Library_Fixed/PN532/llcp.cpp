...
int8_t LLCP::activate(uint8_t *uid, uint16_t timeout)
{
    return link.activateAsTarget(uid, timeout);
}

void LLCP::printHex(const uint8_t *data, const uint32_t numBytes)
{
    link.printHex(data,numBytes);
}

bool LLCP::initPassive(uint8_t cardbaudrate, uint8_t *uid, uint8_t *uidLength, uint16_t timeout, bool inlist)
{
    bool status;
    DMSG_STR("--------------- readPassiveTargetID Started -----------------");
    do
    {
        status = link.initPassive(cardbaudrate,uid,uidLength,timeout,inlist);
        DMSG("STATUS:");
        DMSG_STR(status);
        if (!status)
        {
            DMSG_STR("LLCP initPassive fail!");
            return false;
        }
        else{
            break;
    ...
    DMSG_STR("LLCP initPassive successful!");
    return true;
}

bool LLCP::activateTarget(uint16_t timeout)
{
    DMSG_STR("Attempting to set target as active");
    do
    {
        if (!link.activateTarget(timeout))
        {
            DMSG("LLCP activateTarget fail!");
            return false;
        }
        else{
            break;
    ...
    DMSG_STR("LLCP activateTarget successful!");
    return true;
}

bool LLCP::exchange(uint8_t *send, uint8_t sendLength, uint8_t *response, uint8_t *responseLength)
{
    DMSG_STR("Attempting to exchange data with target");
    do
    {
        if (!link.exchange(send,sendLength,response,responseLength))
        {
            DMSG_STR("LLCP exchange fail!");
            return false;
        }
        else{
            break;
    ...
    DMSG_STR("LLCP exchange successful!");
    return true;
}

int16_t LLCP::waitForConnection(uint8_t *buf, uint8_t bufLength, uint16_t timeout){
    uint16_t len;
    DMSG_STR("WAITING FOR CONNECTION/EXCHANGE");
    len = link.read(buf,bufLength);
    if(0 > len)
    {
        DMSG_STR("READ BUFFER WAS EMPTY");
        return -1;
    } 
    return len;
}
...
bool LLCP::finish(const uint8_t *rbuf, uint8_t hlen, const uint8_t *body, uint8_t blen)
{
    //uint8_t buf[3];
    if (!link.write(rbuf, hlen, body, blen))
    {
        return false;
    }
    DMSG_STR("llcp::finish success!");
    return true;
}
